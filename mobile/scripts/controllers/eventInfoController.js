App.controller('eventInfoController', function($scope, RESTService, $stateParams, $rootScope, $q, $mdBottomSheet, $mdDialog, $location, $cordovaInAppBrowser, $mdToast, Load, Events, Directory, Session){
        routeChange();
        Events.get();
        Directory.get();
        $scope.going = false;
        ...
        function eventOptionsCtrl($scope, $mdBottomSheet, $mdDialog) {
            ...
            $scope.itemClick = function(item){
                switch(item.name){
                    ...
                    case 'SAVE': saveCordovaEvent(); break;
                }
                $mdDialog.hide();
            }
        ...
        function saveCordovaEvent(){
            var event = $scope.event;
            var start_date = new Date(momentInTimezone($scope.event.time_start).format('MM/DD/YYYY hh:mm A'));  
            console.log('start date',start_date);
            var end_date = new Date(momentInTimezone($scope.event.time_end).format('MM/DD/YYYY hh:mm A'));
            var title = event.title.replace(/(\r\n|\n|\r)/gm," ");
            var location = event.address;
            if (event.description){
                var notes = event.description.replace(/(\r\n|\n|\r)/gm," ");
            }
            var success = function(){
                $mdToast.show($mdToast.simple().content('Event Saved'));
            }
            var error = function(){
                $mdToast.show($mdToast.simple().content('Unknown error. Event not saved.'));
            }
            window.plugins.calendar.createEvent(title,location,notes,start_date,end_date,success,error);
        }
    ...
    $scope.openMaps = function(address){
        ...
        $cordovaInAppBrowser.open('maps:q='+encodeURIComponent(address), '_system', options)
          .then(function(event) {
          })
