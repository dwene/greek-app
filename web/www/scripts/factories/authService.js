App.factory('AuthService', function ($http, Session, LoadScreen, $location, $q, RESTService, $rootScope, AUTH_EVENTS) {
    ...
    return RESTService
      ...
      .success(function (data) {
        if (!RESTService.hasErrors(data)){
            ...
            $.cookie(USER_NAME, credentials.user_name.toLowerCase(), {expires: new Date(data.expires)});
            $.cookie(TOKEN, data.token, {expires: new Date(data.expires)});
            Session.create(credentials.user_name, data.token, data.perms);
  ...
  authService.cachedLogin = function(){
    ...
    if (authService.isAuthenticated()){
        ...
        $http.post(ENDPOINTS_DOMAIN + '/_ah/api/netegreek/v1/user/check_login', to_send)
            .success(function(data){
              ...
              if (!authService.isAuthenticated()){
                if (!RESTService.hasErrors(data)) { 
                      ...
                      Session.create($.cookie(USER_NAME),$.cookie(TOKEN),'council');
